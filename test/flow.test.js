...
const { api, setup, teardown, signUp, auth, futureDate } = require('./helpers');
const { User, Cycle, Transaction } = require('../src/models');
const cyclesSvc = require('../src/services/cycles');
...
async function pay(token, cycleId, amount, gatewayAmount) {
...
async function createGroup() {
  ...
  const byUser = Object.fromEntries(setupView.body.members.map((m) => [m.userId, m.membershipId]));
...
test('full rotation: create, pay (full + partial), payout, next cycle', async () => {
  ...
  const all = hist.body.months.flatMap((m) => m.transactions);
  ...
  assert.ok(notes.body.notifications.some((n) => n.type === 'payout_sent'));
  const act = await api().get(`/api/v1/groups/${gid}/activity`).set(auth(m2.token));
  assert.ok(act.body.activity.some((a) => a.eventType === 'payout_sent'));
...
test('failed payout → delayed recovery → sent', async () => {
  ...
  require('../src/services/squadco').setMockAccountName('0100000999', 'GBENGA MEMBER');
  ...
  const ids = Object.fromEntries(sv.body.members.map((x) => [x.userId, x.membershipId]));
  ...
  require('../src/services/squadco').setMockAccountName('0100000123', 'GBENGA MEMBER');
