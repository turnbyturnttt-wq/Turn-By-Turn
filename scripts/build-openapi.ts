...
/** A JSON-schema fragment. Kept loose on purpose: this file only assembles documentation. */
...
const s = (extra: Schema = {}): Schema => ({ type: 'string', ...extra });
const i = (description?: string): Schema => ({ type: 'integer', ...(description ? { description } : {}) });
const b = (): Schema => ({ type: 'boolean' });
const arr = (items: Schema): Schema => ({ type: 'array', items });
const kobo = (description = 'Amount in kobo (₦1 = 100)'): Schema => ({ type: 'integer', description });
...
const E: Endpoint[] = [];
const add = (method: Method, p: string, tag: string, summary: string, o: EndpointOptions = {}): void => {
  E.push({ method, p, tag, summary, ...o });
};
...
function errResp(codes: ErrorCode[]): Record<string, { description: string; content: Schema }> {
  const out: Record<string, { description: string; content: Schema }> = {};
  for (const c of codes) {
    const st = String(ErrorCodes[c].status);
    const entry = (out[st] ??= { description: '', content: { 'application/json': { schema: ref('Error') } } });
    entry.description = entry.description ? `${entry.description}, ${c}` : c;
  }
  return out;
}

const paths: Record<string, Partial<Record<Method, Schema>>> = {};
for (const e of E) {
  const params: Schema[] = [...e.p.matchAll(/\{(\w+)\}/g)].map((m) => ({ name: m[1], in: 'path', required: true, schema: s() }));
  ...
  const codes = [
    ...new Set<ErrorCode>([...(e.errors ?? []), 'VALIDATION_ERROR', ...(needsAuth ? (['UNAUTHENTICATED', 'SESSION_EXPIRED'] as const) : [])]),
  ];
  const op: Schema = {
    tags: [e.tag],
    summary: e.summary,
    operationId: `${e.method}${e.p.replace(/[{}]/g, '').replace(/[^a-zA-Z0-9]+(.)?/g, (_m: string, c?: string) => (c ? c.toUpperCase() : ''))}`,
