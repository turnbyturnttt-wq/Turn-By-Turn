/**
 * Squadco (https://docs.squadco.com) integration: payment collection, payout transfers,
 * bank account name lookup, SMS OTP and webhook signature verification.
 *
 * When SQUADCO_MOCK is on (the default when no secret key is configured) every call is
 * simulated so the Flutter team can exercise the full flow locally; see routes/dev.ts.
 */

import crypto from 'crypto';
import { config } from '../config/env';
import { ApiError, err } from '../utils/errors';
import type { Kobo } from '../utils/money';
import bankList from '../content/banks.json';

export interface Bank {
  code: string;
  name: string;
}

export const banks: readonly Bank[] = bankList;

const cfg = config.squadco;

type SquadBody = Record<string, unknown> & { status?: number | string; success?: boolean; message?: string; data?: Record<string, unknown> };

class GatewayHttpError extends Error {
  constructor(
    message: string,
    readonly gatewayStatus: number,
    readonly gatewayBody: unknown,
  ) {
    super(message);
  }
}

async function request(method: 'GET' | 'POST', path: string, body?: unknown): Promise<SquadBody> {
  let res: Response;
  try {
    res = await fetch(`${cfg.baseUrl}${path}`, {
      method,
      headers: { Authorization: `Bearer ${cfg.secretKey}`, 'Content-Type': 'application/json' },
      body: body ? JSON.stringify(body) : undefined,
      signal: AbortSignal.timeout(30000),
    });
  } catch (e) {
    throw err('GATEWAY_ERROR', undefined, { details: { cause: (e as Error).message } });
  }
  const json = (await res.json().catch(() => ({}))) as SquadBody;
  if (!res.ok || (json.status !== undefined && Number(json.status) >= 400) || json.success === false) {
    throw new GatewayHttpError(json.message ?? `Squadco ${method} ${path} failed with ${res.status}`, res.status, json);
  }
  return json;
}

const str = (v: unknown): string | undefined => (typeof v === 'string' ? v : undefined);

export function bankByCode(code: string): Bank | null {
  return banks.find((b) => b.code === code) ?? null;
}

// ------------------------------------------------------------------ Account name lookup

export interface ResolvedAccount {
  accountName: string;
  bankName: string;
  bankCode: string;
}

/** Mock lookups return the name registered via setMockAccountName, else a stable placeholder. */
const mockNames = new Map<string, string>();
export function setMockAccountName(accountNumber: string, name: string): void {
  mockNames.set(accountNumber, name);
}
function mockAccountName(accountNumber: string): string {
  return mockNames.get(accountNumber) ?? `TEST ACCOUNT ${accountNumber.slice(-4)}`;
}

/** Resolves the registered account name for a NUBAN. Throws BANK_VERIFICATION_FAILED. */
export async function resolveAccount(bankCode: string, accountNumber: string): Promise<ResolvedAccount> {
  const bank = bankByCode(bankCode);
  if (!bank) throw err('BANK_VERIFICATION_FAILED', 'Select a supported bank.');
  if (!/^\d{10}$/.test(accountNumber)) throw err('BANK_VERIFICATION_FAILED', 'Account number must be 10 digits.');

  if (cfg.mock) {
    // Deterministic fake: numbers ending 000 fail, which lets the app test the error screen.
    if (accountNumber.endsWith('000')) throw err('BANK_VERIFICATION_FAILED');
    return { accountName: mockAccountName(accountNumber), bankName: bank.name, bankCode };
  }
  try {
    const json = await request('POST', '/payout/account/lookup', { bank_code: bankCode, account_number: accountNumber });
    const accountName = str(json.data?.account_name) ?? str(json.data?.accountName);
    if (!accountName) throw new Error('No account name returned');
    return { accountName: accountName.trim(), bankName: bank.name, bankCode };
  } catch (e) {
    if (e instanceof ApiError && e.code === 'GATEWAY_ERROR') throw e;
    throw err('BANK_VERIFICATION_FAILED');
  }
}

// ------------------------------------------------------------------ Payments

export interface InitiatePaymentInput {
  reference: string;
  amountKobo: Kobo;
  email: string;
  customerName: string;
  metadata?: Record<string, string>;
}

/** Starts a Squadco checkout. Amount is in kobo (Squadco also works in kobo). */
export async function initiatePayment(input: InitiatePaymentInput): Promise<{ checkoutUrl: string; gatewayReference: string }> {
  if (cfg.mock) {
    return {
      checkoutUrl: `${config.publicBaseUrl}/api/v1/dev/checkout/${encodeURIComponent(input.reference)}`,
      gatewayReference: input.reference,
    };
  }
  let json: SquadBody;
  try {
    json = await request('POST', '/transaction/initiate', {
      amount: input.amountKobo,
      email: input.email,
      currency: 'NGN',
      initiate_type: 'inline',
      transaction_ref: input.reference,
      customer_name: input.customerName,
      callback_url: cfg.paymentCallbackUrl || undefined,
      pass_charge: false,
      metadata: input.metadata,
    });
  } catch (e) {
    if (e instanceof ApiError) throw e;
    throw err('GATEWAY_ERROR', undefined, { details: { cause: (e as Error).message } });
  }
  const checkoutUrl = str(json.data?.checkout_url);
  if (!checkoutUrl) throw err('GATEWAY_ERROR', undefined, { details: { cause: 'No checkout_url returned' } });
  return { checkoutUrl, gatewayReference: str(json.data?.transaction_ref) ?? input.reference };
}

export type PaymentOutcome =
  | { status: 'success'; amountKobo?: Kobo | null; channel?: string }
  | { status: 'failed'; reason?: string }
  | { status: 'pending' };

/** Asks Squadco for the final state of a payment. */
export async function verifyPayment(reference: string): Promise<PaymentOutcome> {
  if (cfg.mock) return { status: 'pending' };
  const json = await request('GET', `/transaction/verify/${encodeURIComponent(reference)}`);
  const d = json.data ?? {};
  const s = String(d.transaction_status ?? '').toLowerCase();
  if (s === 'success') {
    return { status: 'success', amountKobo: d.transaction_amount != null ? Number(d.transaction_amount) : null, channel: str(d.transaction_type) };
  }
  if (['failed', 'abandoned', 'cancelled'].includes(s)) return { status: 'failed', reason: `Payment ${s}` };
  return { status: 'pending' };
}

// ------------------------------------------------------------------ Payouts (transfers)

export type TransferOutcome =
  | { status: 'sent' }
  | { status: 'processing' }
  | { status: 'failed'; failureReason?: string };

/** Squadco requires transfer references to be prefixed with the merchant id. */
export function transferReference(ref: string): string {
  return cfg.merchantId ? `${cfg.merchantId}_${ref}` : ref;
}

export interface TransferInput {
  reference: string;
  amountKobo: Kobo;
  bankCode: string;
  accountNumber: string;
  accountName: string;
  remark?: string;
}

export async function transfer(input: TransferInput): Promise<TransferOutcome> {
  if (cfg.mock) {
    // Accounts ending 999 simulate a failed transfer so the Failed / Delayed Recovery screens can be exercised.
    if (input.accountNumber.endsWith('999')) return { status: 'failed', failureReason: 'Beneficiary bank unavailable' };
    return { status: 'sent' };
  }
  try {
    const json = await request('POST', '/payout/transfer', {
      transaction_reference: transferReference(input.reference),
      amount: String(input.amountKobo),
      bank_code: input.bankCode,
      account_number: input.accountNumber,
      account_name: input.accountName,
      currency_id: 'NGN',
      remark: (input.remark ?? 'TurnByTurn payout').slice(0, 100),
    });
    return interpretTransferStatus(json);
  } catch (e) {
    // A timeout or 5xx leaves the transfer outcome unknown: keep it processing and requery.
    if (!(e instanceof GatewayHttpError) || e.gatewayStatus >= 500) return { status: 'processing' };
    return { status: 'failed', failureReason: e.message };
  }
}

export async function requeryTransfer(reference: string): Promise<TransferOutcome> {
  if (cfg.mock) return { status: 'sent' };
  try {
    const json = await request('POST', '/payout/requery', { transaction_reference: transferReference(reference) });
    return interpretTransferStatus(json);
  } catch (e) {
    if (e instanceof GatewayHttpError && e.gatewayStatus === 404) {
      return { status: 'failed', failureReason: 'Transfer not found at gateway' };
    }
    return { status: 'processing' };
  }
}

function interpretTransferStatus(json: SquadBody): TransferOutcome {
  const d = json.data ?? {};
  const s = String(d.transaction_status ?? d.status ?? json.message ?? '').toLowerCase();
  if (s.includes('success') || s.includes('completed')) return { status: 'sent' };
  if (s.includes('fail') || s.includes('revers') || s.includes('declin')) {
    return { status: 'failed', failureReason: str(d.response_description) ?? json.message ?? 'Transfer failed' };
  }
  return { status: 'processing' };
}

// ------------------------------------------------------------------ SMS

export async function sendSms(phone: string, message: string): Promise<{ mocked: boolean }> {
  if (cfg.mock) {
    if (!config.isTest) console.info(`[squadco:mock-sms] to=${phone} message="${message}"`);
    return { mocked: true };
  }
  await request('POST', '/sms/send/instant', {
    sender_id: cfg.smsSenderId,
    messages: [{ phone_number: phone.replace(/^\+/, ''), message }],
  });
  return { mocked: false };
}

// ------------------------------------------------------------------ Webhooks

/**
 * Squadco signs webhook bodies with HMAC-SHA512 of the raw body using the secret key and sends
 * the uppercase hex digest in `x-squad-encrypted-body`.
 */
export function verifyWebhookSignature(rawBody: string, signature: string | undefined): boolean {
  if (cfg.mock && !cfg.secretKey) return true;
  if (!signature || !rawBody) return false;
  const expected = crypto.createHmac('sha512', cfg.secretKey).update(rawBody).digest('hex').toUpperCase();
  const a = Buffer.from(expected);
  const b = Buffer.from(signature.toUpperCase());
  return a.length === b.length && crypto.timingSafeEqual(a, b);
}
